...
server.get('/', checkAuth, (request, response) => {
  ...
  let citiesForRender = db.cities

  if (usersQuery.limit !== undefined && Number.isNaN(+usersQuery.limit) === false) {
    citiesForRender = db.cities.slice(0, usersQuery.limit)
  } else
    if (usersQuery.reverse !== undefined && usersQuery.reverse === 'true') {
      citiesForRender = db.cities.reverse()
    }

  response.render('main', { listOfCities: citiesForRender })
})

server.delete('/post', (req, res) => {
  const userId = req.session?.user?.id // айди автора в сессии
  const { postId } = req.body // айди поста
  const rightId = db.cities.find((el) => Number(el.postNumber) === Number(postId))
  if (rightId) {
    if (userId === rightId.imageId) {
      const newcities = db.cities.filter((el) => Number(el.postNumber) !== postId)
      db.cities = newcities
